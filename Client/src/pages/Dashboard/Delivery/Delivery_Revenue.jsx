...
function Delivery_Revenue() {
  ...
  const [MultiData5, setMultiData5] = useState([]);
  ...
  const fetchTData = () => {
    ...
    fetch(`http://${API_URL}/Admin/TodayR2/?date=${date}&delivery=1`, {
      method: "GET",
    })
  ...
  const fetch_Week_Data = () => {
    fetch(`http://${API_URL}/Admin/WeeklyR2/?delivery=1`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((responseJson) => {
  ...
  const fetch_Month_Data = () => {
    fetch(`http://${API_URL}/Admin/MonthlyR2/?delivery=1`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((responseJson) => {
  ...
  const fetch_Year_Data = () => {
    fetch(`http://${API_URL}/Admin/YearlyR2/?delivery=1`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((responseJson) => {
  ...
  const fetchMultiLineData = () => {
    fetch(`http://${API_URL}/Admin/DividedHourlyR2/?delivery=1`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((responseJson) => {
        ...
        setMultiData5(responseJson.delivery);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const fetchWData = () => {
    fetch(`http://${API_URL}/Admin/WeekR2/?delivery=1`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((responseJson) => {
  ...
  const fetchMData = () => {
    ...
    fetch(`http://${API_URL}/Admin/MonthR2/?month=${month}&year=${year}&delivery=1`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((responseJson) => {
  ...
  const fetchYData = () => {
    ...
    fetch(`http://${API_URL}/Admin/YearR2/?year=${year}&delivery=1`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((responseJson) => {
  ...
  return (
    <div className="bg-white flex flex-col  h-[100%] ">
      <h1 className="text-2xl sm:text-4xl text-start items-start mt-2 ml-2 text-black font-bold">
        Real Data
        <RecyclingRounded onClick={() => navigate("/revenue2")} />
      </h1>
...
export default Delivery_Revenue;
