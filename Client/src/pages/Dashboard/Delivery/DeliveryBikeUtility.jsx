  ...
  const fetchDetails = async () => {
    try {
      const response = await fetch(
        `http://${API_URL}/Delivery-Admin/Delivery-BikeUtility/${selectedDate}/`
      );
      const data = await response.json();
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-y-10 sm:gap-x-50  ml-3 xxs:ml-5 mr-6 lg:ml-24  items-center justify-center xxs:w-screen sm:w-full  sm:grid-cols-3  mt-10 ">
            ...
            <StatBox
              title="EV Killometer"
              value={
                evBikes.reduce((sum, item) => sum + item.km_for_day, 0) + "  KM"
              }
            />
            <StatBox
              ...
              name="hours"
              value={
                petrolBikes.reduce((sum, item) => sum + item.km_for_day, 0) +
                "  KM"
              }
            />
...
export default DeliveryBikeUtility;
